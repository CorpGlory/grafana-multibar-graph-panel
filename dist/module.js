...
var GraphRenderer = /** @class */ (function () {
    ...
    GraphRenderer.prototype._addTimeAxis = function (minTimeStep) {
        ...
        if (this.panel.bars && this.sortedSeries.length > 0 && this.sortedSeries[0].datapoints.length > 0) {
            ...
            var generatedTicks = this._generateTicks(groupsAmount, ticks, max, minTimeStep);
            if (generatedTicks.length !== 0) {
                var format_1 = this._timeFormat(ticks, min, max);
                var formatDate_1 = jquery__WEBPACK_IMPORTED_MODULE_3___default.a.plot.formatDate;
                ticks = lodash__WEBPACK_IMPORTED_MODULE_19___default.a.map(generatedTicks, function (tick) { return [
                    tick[0],
                    formatDate_1(new Date(tick[1]), format_1)
                ]; });
            }
        }
    ...
    GraphRenderer.prototype._generateTicks = function (groupsAmount, maxTicks, rangeTo, timeStep) {
        var ticks = [];
        var groups = Math.max(this.sortedSeries[0].datapoints.length, groupsAmount);
        var multiplier = Math.floor(groups / maxTicks) || 1;
        var firstGroupTimestamp = this.sortedSeries[0].datapoints[0][1];
        var offset;
